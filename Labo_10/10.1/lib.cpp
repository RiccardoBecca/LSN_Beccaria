...
Cities::Cities(int square_circle, int numero, Random rand){
    ...
    ifstream input;
    if(square_circle==0){
        input.open("Cities_circle.txt");
    }
    else{
        input.open("Cities_square.txt");
    }
...
Generazione::Generazione(int lunghezza, int num_figli, double cross, double m1, double m2, double m3, double m4, Random rand, int T){
...
void Generazione::Mutation_swap(Cities city){
    for(int i=0; i< m_gen.size(); i++){
        
        //double alpha=(double) rand() / (RAND_MAX);
        double alpha=m_rand.Rannyu();
        
        if(alpha<m_prob_m1){ //SWAP
            //int a = int(((double) rand() / (RAND_MAX))*(m_lunghezza-1) +1);
            //int b = int(((double) rand() / (RAND_MAX))*(m_lunghezza-1) +1);
            int a = int((m_rand.Rannyu())*(m_lunghezza-1) +1);
            int b = int((m_rand.Rannyu())*(m_lunghezza-1) +1);
...
void Generazione::Mutation_permut(Cities city){
    for(int i=0; i<m_gen.size();i++){
        double alpha=m_rand.Rannyu();
        if(alpha<m_prob_m2){ //PERMUTATION
...
void Generazione::Mutation_shift(Cities city){
    for(int i=0; i<m_gen.size();i++){
        double alpha=m_rand.Rannyu();
        if(alpha<m_prob_m3){ //SHIFT
...
void Generazione::Mutation_inversion(Cities city){
    for(int i=0;i<m_gen.size(); i++){
        double alpha=m_rand.Rannyu();
        if(alpha<0.08){ //INVERSION
            Figlio copy(m_lunghezza);
...
void Generazione::Metro(Cities city){
    Figlio old_son(m_gen[0].Size());
    //cout << endl << endl << "COME FROM "<< endl;
    //m_gen[0].Print();
    //cout << "BEFORE " << endl;
    //old_son.Print();
    for(int i=0; i<old_son.Size(); i++){
        old_son.Set(i, m_gen[0].Get(i));
    }
    old_son.Setlen(m_gen[0].Getlen());
    //cout << "AFTER " << endl;
    //old_son.Print();
    ...
    //m_gen[0].Print();
    //cout << endl;
    double r=m_rand.Rannyu(0,1);
    double a=Min(1,exp(-(m_gen[0].Getlen()-old_son.Getlen())/(m_T)));
    //cout << a << endl;
    if(r<a){
        //cout << "DEVI ACCETTARE E QUINDI LASCIARE COSI' E AUMENTARE INT CHE CONTEGGIA" << endl;
        m_accepted+=1;
    }
    else{
        //cout << "DEVI FAR TORNARE m_gen[0] devi farlo tornare essere F1" << endl;
        for(int i=0; i<m_gen[0].Size();i++){
            m_gen[0].Set(i, old_son.Get(i));
        }
    }
}

void Generazione::Metro_swap(Cities city){
    Figlio old_son(m_gen[0].Size());
    //cout << endl << endl << "COME FROM "<< endl;
    //m_gen[0].Print();
    //cout << "BEFORE " << endl;
    ...
    if(r<a){
        ...
        for(int i=0; i<m_gen[0].Size();i++){
            m_gen[0].Set(i, old_son.Get(i));
            m_gen[0].Setlen(old_son.Getlen());
        }
    }
}

void Generazione::Metro_permut(Cities city){
    Figlio old_son(m_gen[0].Size());
    //cout << endl << endl << "COME FROM "<< endl;
    //m_gen[0].Print();
    //cout << "BEFORE " << endl;
...
void Generazione::Metro_shift(Cities city){
    Figlio old_son(m_gen[0].Size());
    //cout << endl << endl << "COME FROM "<< endl;
    //m_gen[0].Print();
    //cout << "BEFORE " << endl;
...
void Generazione::Metro_inversion(Cities city){
    Figlio old_son(m_gen[0].Size());
    //cout << endl << endl << "COME FROM "<< endl;
    //m_gen[0].Print();
    //cout << "BEFORE " << endl;
